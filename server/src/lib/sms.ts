...
import { phoneToSmsDigits } from './phone.js';
...
async function postNikita(to: string, text: string, sender: string) {
...
async function sendViaNikita(to: string, text: string) {
  const sender =
    (process.env.SMS_SENDER?.trim() || 'MuAlim').replace(/[^A-Za-z0-9.-]/g, '').slice(0, 11) || 'MuAlim';
  try {
    await postNikita(to, text, sender);
  } catch (err) {
    if ((err as { nikitaStatus?: number }).nikitaStatus === 5) {
      await postNikita(to, text, phoneToSmsDigits(to).slice(0, 14));
      return;
    }
    throw err;
  }
...
export async function sendPasswordResetSms(to: string, code: string) {
  if (!isSmsConfigured()) {
